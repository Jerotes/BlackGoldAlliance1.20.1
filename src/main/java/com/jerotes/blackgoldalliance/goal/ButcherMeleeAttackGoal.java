...
public class ButcherMeleeAttackGoal extends JerotesMeleeAttackGoal {
    protected final BlackGoldButcherEntity mob;
    ...
    private static final long COOLDOWN_BETWEEN_CAN_USE_CHECKS = 30L;

    public ButcherMeleeAttackGoal(BlackGoldButcherEntity pathfinderMob, boolean bl) {
        super(pathfinderMob, 1.2f, bl);
    ...
    @Override
    public void start() {
        this.mob.getNavigation().moveTo(this.path, mob.getRunSpeed() * (mob.getChainsawTick() > 0 ? 1.25 : 1));
    ...
    public void tick() {
        ...
        double speed = mob.getRunSpeed() * (mob.getChainsawTick() > 0 ? 1.25 : 1);
        ...
        if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(livingEntity)) && this.ticksUntilNextPathRecalculation <= 0 && (this.pathedTargetX == 0.0 && this.pathedTargetY == 0.0 && this.pathedTargetZ == 0.0 || livingEntity.distanceToSqr(this.pathedTargetX, this.pathedTargetY, this.pathedTargetZ) >= 0 || this.mob.getRandom().nextFloat() < 0.05f)) {
            ...
            if (!this.mob.getNavigation().moveTo(livingEntity, speed * (mob.getChainsawTick() > 0 ? 1.25 : 1))) {
